...
import { ArrowRight, ArrowLeft, Check, Zap, X, Plus } from "lucide-react";
...
export function Register() {
  ...
  const validateStep2 = () => {
    const valid = offeredSkills.some((s) => s.name && s.experience);
    if (!valid) setErrors({ skills: "Add at least one complete skill" });
    else setErrors({});
    return valid;
  };
  ...
  const handleSubmit = async () => {
    ...
    const data: RegisterData = {
      ...
      password,
      bio,
      skillsOffered: offeredSkills.filter((s) => s.name && s.experience),
      skillsWanted,
    };
    const result = await register(data);
    setIsLoading(false);
    if (result.success) navigate("/dashboard");
    else setSubmitError(result.error || "Unable to create account.");
  };
  ...
  const handleGoogle = async () => {
    ...
    if (result.success) navigate("/dashboard");
    else setSubmitError(result.error || "Google sign-up is unavailable.");
  };
      ...
      <div className="hidden lg:flex lg:w-80 bg-gradient-to-br from-indigo-600 via-violet-600 to-purple-700 flex-col justify-between p-10 relative overflow-hidden">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3 mb-12">
            ...
            <span className="text-white" style={{ fontSize: "22px", fontWeight: 700 }}>SkillSwap</span>
          </div>
              ...
              return (
                <div key={label} className="flex items-center gap-4">
                  ...
                  <div>
                    ...
                    {isCurrent && (
                      <p className="text-indigo-200" style={{ fontSize: "11px" }}>Current step</p>
                    )}
                  </div>
                </div>
              );
      ...
      <div className="flex-1 flex items-start justify-center py-12 px-6 overflow-auto">
        <div className="w-full max-w-lg">
          ...
          <div className="lg:hidden flex items-center gap-2 mb-8">
            ...
            <span className="text-slate-900" style={{ fontSize: "20px", fontWeight: 700 }}>SkillSwap</span>
          </div>
          ...
          <div className="mb-8">
            <p className="text-indigo-500 mb-1" style={{ fontSize: "12px", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.06em" }}>
          ...
          {step === 1 && (
            <div className="space-y-4">
              <button
                ...
                className="w-full flex items-center justify-center gap-3 py-3 px-4 rounded-xl border border-slate-200 bg-white text-slate-700 hover:bg-slate-50 hover:border-slate-300 transition-all disabled:opacity-60"
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-slate-400" style={{ fontSize: "12px" }}>or with email</span>
                <div className="flex-1 h-px bg-slate-200" />
              </div>

              <div>
                <label className="block text-slate-700 mb-1.5" style={{ fontSize: "13px", fontWeight: 500 }}>Full Name</label>
                <input
                  type="text"
                  value={name}
              ...
              <div>
                <label className="block text-slate-700 mb-1.5" style={{ fontSize: "13px", fontWeight: 500 }}>Email</label>
                <input
                  type="email"
                  value={email}
              ...
              <div>
                <label className="block text-slate-700 mb-1.5" style={{ fontSize: "13px", fontWeight: 500 }}>Password</label>
                <input
                  type="password"
                  value={password}
              ...
              {offeredSkills.map((skill, idx) => (
                <div key={idx} className="bg-white rounded-2xl border border-slate-200 p-4 space-y-3">
                  ...
                  <div className="relative">
                    <label className="block text-slate-500 mb-1" style={{ fontSize: "12px" }}>Skill Name</label>
                    <input
                      type="text"
                      value={skill.name}
                      onChange={(e) => {
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div>
                      <label className="block text-slate-500 mb-1" style={{ fontSize: "12px" }}>Level</label>
                      <select
                        value={skill.level}
                    ...
                    <div>
                      <label className="block text-slate-500 mb-1" style={{ fontSize: "12px" }}>Experience</label>
                      <input
                        type="text"
                  ...
                  <div className="flex flex-wrap gap-2">
                    {["React", "Python", "UI/UX Design", "Machine Learning", "Digital Marketing", "Data Analysis", "Figma", "AWS", "Flutter", "Node.js"].map((skill) => (
                      <button
                        ...
                        style={{ fontSize: "12px" }}
                      >
                        {wantedSkills.includes(skill) ? "✓ " : "+ "}{skill}
                      </button>
                    ))}
                  </div>
                </div>
